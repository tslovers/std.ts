 ...
 */
function quickSort(a: any[], p: number, r: number,
 ...
 */
function qSort(a: any[], p: number, r: number, compare?: criterion) {
  if (p < r) {
    const q = partition(a, p, r, compare);
 ...
 */
function partition(a: any[], p: number, r: number,
                   compare: criterion = ascendingComparison): number {
  let i = p - 1;
 ...
 */
function swapPositions(a: any[], i: number, j: number) {
  const temp = a[i];
  a[i] = a[j];
  a[j] = temp;
}
